...
- [PGSQL Statistics System – How a DBA Actually Uses It](#pgsql-statistics-system--how-a-dba-actually-uses-it)
  ...
  - [Confusion between System Catalogs (**`pg_*`**) and System Statistics (**`pg_stat_*`**)](#confusion-between-system-catalogs-pg_-and-system-statistics-pg_stat_)
...
## Confusion between System Catalogs (**`pg_*`**) and System Statistics (**`pg_stat_*`**)

- **System catalogs** (**`pg_*`**) store what exists in the database. They keep details like which databases, tables, columns, indexes, and users are present. This information is mostly stable and changes only when you create, drop, or alter something.

<br>

- **Statistics views** (**`pg_stat_*`**) show what is happening in the database. They track running activity like which queries are active, how many rows are read, whether indexes are used, and if something is waiting.

<br>

- **In simple terms:**
  - **`pg_*`** tells you the structure of the database.
  - **`pg_stat_*`** tells you how the database is behaving right now.
...
- The statistics system is <u><b>how PostgreSQL keeps track of what it is doing while it runs</b></u>.

<br>

- Each backend process records things like <u><b>what query it is working on</b></u>, <u><b>how many rows it reads</b></u>, <u><b>whether data comes from memory or disk</b></u>, and <u><b>if it is waiting for a lock</b></u>. This information is <u><b>kept in shared memory</b></u> and shown through the **`pg_stat_*`** views.

<br>

- These views do not show configuration or settings. They show <u><b>what is actually happening inside the database</b></u>. That’s why when the database feels slow or stuck, the answer is often already there.
...
- **`pg_stat_activity`** shows what is happening in the database right now.

- It lists all connected sessions and tells you <u><b>which queries are running</b></u>, <u><b>which ones are idle</b></u>, <u><b>how long they’ve been running</b></u>, and <u><b>whether they are waiting for something like a lock</b></u>. When the database feels slow or blocked, this is usually the first place to look.

<br>

<br>
<details>
<summary><mark><b>Columns of pg_stat_activity</b></mark></summary>
<br>

- **datid:** internal ID of the database the session is connected to.
- **datname:** name of the database.
- **pid:** process ID of the backend handling this session
- **usename:** name of the connected user.
- **application_name:** name of the client app (psql, app, etc.)
- **client_addr:** IP address of the client
- **client_hostname:** hostname of the client
- **client_port:** port number used by the client
- **backend_start:** time when this session connected to PGSQL.
- **xact_start:** time when the current transaction started.
- **query_start:** time when the current query started
- **wait_event_type:** category of what the session is waiting for
- **wait_event:** exact thing the session is waiting on
- **state:** current state of the session (active, idle, etc)
- **backend_xid:** transaction ID currently used by this session
- **backend_xmin:** oldest transaction ID this session depends on
- **query_id:** internal ID of the query (used for query tracking)
- **query:** the SQL query currently running or last executed
- **backend_type:** type of backend (client, autovacuum, worker, etc.)
...
- Common values:
  ...
  - **`Client:`** waiting for client input / output.
